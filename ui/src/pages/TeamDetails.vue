  ...
  <q-page class="q-pa-md q-gutter-md">
    ...
    <div class="q-pb-md q-gutter-sm">
      <q-breadcrumbs>
        ...
        <q-breadcrumbs-el label="Team Details" />
      </q-breadcrumbs>
    </div>
    ...
    <div class="q-gutter-md">
      <q-form @submit="onSubmit" @reset="onReset" class="q-gutter-md">
        ...
        <q-input filled v-model="teamName" label="Team name" lazy-rules
          :rules="[val => val && val.length > 0 || 'Please enter team name']" />
        <q-input filled v-model="snapshotId" label="Snapshot ID" lazy-rules
          :rules="[val => val && val.length > 0 || 'Please enter snapshot id']" />
    ...
    <PanelSection title="Users">
      <div >
        <q-input filled v-model="membershipFilter" label="Filter users" debounce="300" clearable />
...
const onSubmit = () => {
  ...
  axios({
    ...
    memberships.value = res.data.data.members
  }).catch(e => {
    $q.notify("Error fetching team details");
    console.error(e);
  }).finally(() => {
    bar.value.stop();
  })
